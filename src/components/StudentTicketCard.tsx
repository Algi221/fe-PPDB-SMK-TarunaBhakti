...
import { ChevronLeft, CheckCircle, Clock, XCircle, User, MapPin } from "lucide-react";

export interface StudentTicketData {
  id: number;
  nama: string;
  nisn: string;
  sekolah_asal?: string;
...
interface StudentTicketCardProps {
  student: StudentTicketData;
  onBack: () => void;
}

export default function StudentTicketCard({ student, onBack }: StudentTicketCardProps) {
  return (
            ...
            <div className="flex flex-col gap-2 items-end w-fit">
              <div className="relative group/badge cursor-default">
                ...
                <span className={`relative px-4 py-1.5 border text-[10px] font-black uppercase tracking-[0.2em] rounded-full flex items-center gap-2 shadow-sm backdrop-blur-md ${
                  student.status === 'Approved' ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-500/30' :
                  student.status === 'Rejected' ? 'bg-rose-50 dark:bg-rose-500/10 text-rose-750 dark:text-rose-300 border-rose-200 dark:border-rose-500/30' :
                  'bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-500/30'
                }`}>
                  {student.status === 'Approved' ? <><CheckCircle size={12} className="text-emerald-500" /> TERVERIFIKASI</> : 
                   student.status === 'Rejected' ? <><XCircle size={12} className="text-rose-500" /> DITOLAK</> : 
              ...
              <div className="relative p-2.5 bg-white dark:bg-slate-900 rounded-3xl shadow-lg border border-slate-200 dark:border-slate-700 group/qr hover:shadow-xl transition-all duration-500 hover:border-blue-400 dark:hover:border-blue-500/50">
                ...
                {(() => {
                  const verifyUrl = typeof window !== 'undefined' 
                    ...
                    : `http://localhost:3000/verify/${student.id}`;
                  return (
                    <div className="bg-white p-1.5 rounded-2xl relative z-10" key={student.id}>
                      <img 
          ...
          <div className="border-t border-slate-200 dark:border-white/10 pt-5 mt-8 flex flex-col sm:flex-row items-center justify-between gap-3 text-[10px] font-bold text-slate-500 dark:text-slate-400 tracking-[0.2em] relative z-10">
            ...
            <div className="flex items-center gap-2">
              <span className="text-blue-700 dark:text-blue-400 font-black text-[9px] uppercase tracking-wider">
                {student.status === 'Approved' ? 'DATA TELAH DIVERIFIKASI PANITIA' : 
                 student.status === 'Rejected' ? 'PENDAFTARAN DITOLAK' : 

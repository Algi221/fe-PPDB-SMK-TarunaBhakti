...
export default function DashboardLayout({ children }) {
  ...
  if (!adminToken) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center text-slate-800 dark:text-white transition-colors duration-300">
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0b0f19] text-slate-800 dark:text-slate-100 flex font-sans overflow-hidden transition-colors duration-300">
      {/* Sidebar Navigation */}
      <aside className="w-64 bg-white dark:bg-slate-900 border-r border-slate-200/80 dark:border-slate-800/60 flex flex-col shrink-0 transition-colors duration-300 z-50">
        {/* Brand Header */}
        ...
        <div className="p-4 border-t border-slate-200/80 dark:border-slate-800/60 bg-slate-50 dark:bg-slate-950/20 flex flex-col gap-3">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-blue-600 to-sky-400 flex items-center justify-center font-black text-white shrink-0 shadow-md">
            ...
            <div className="min-w-0 flex-1">
              ...
              <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold truncate leading-none uppercase tracking-wider block mt-0.5">@{adminUser?.username || "admin_tb"}</span>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="w-full py-2.5 px-3 bg-slate-100 hover:bg-rose-500/10 hover:text-rose-600 dark:bg-white/5 dark:hover:bg-rose-500/10 dark:hover:text-rose-300 text-slate-500 dark:text-slate-400 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 border border-slate-200 dark:border-white/5 hover:border-rose-500/20"
          >
            <LogOut size={14} />
            Keluar Sesi
          </button>
        ...
        <header className="h-16 border-b border-slate-200/80 dark:border-slate-800/60 bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl flex items-center justify-between px-8 shrink-0 z-40 sticky top-0 transition-colors duration-300">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="/"
              target="_blank"
              className="h-10 px-4 bg-blue-600/10 hover:bg-blue-600/20 text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 border border-blue-600/20 hover:border-blue-600/30 rounded-xl text-xs font-extrabold uppercase tracking-wider transition-all flex items-center gap-1.5"
            >
              <Globe size={14} />
              <span>Beranda</span>
            </Link>

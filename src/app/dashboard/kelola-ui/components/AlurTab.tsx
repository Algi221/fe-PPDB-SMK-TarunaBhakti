...
interface AlurTabProps {
  alurList: AlurItem[];
  handleAddAlur: () => void;
  handleUpdateAlur: (id: number, key: keyof AlurItem, val: string | number) => void;
  handleRemoveAlur: (id: number) => void;
  handleMoveAlur: (index: number, direction: "up" | "down") => void;
}

export default function AlurTab({
  alurList,
  handleAddAlur,
  handleUpdateAlur,
  handleRemoveAlur,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/5 pb-4 mb-4">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4.5 py-2.5 rounded-xl bg-slate-50 hover:bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-white/5 text-[10px] uppercase font-bold text-slate-700 dark:text-slate-300 transition-all shadow-sm cursor-pointer"
        >
          <Plus size={14} />
          ...
          >
            ...
            <div className="flex items-center gap-1.5 shrink-0 self-center">
              ...
              <button
                ...
                className={`p-1.5 rounded-lg border border-slate-200 hover:bg-slate-100 dark:border-white/5 dark:hover:bg-slate-900 transition-all ${
                  idx === alurList.length - 1 ? "opacity-30 cursor-not-allowed" : "cursor-pointer"
                }`}
              >
                <ChevronDown size={14} />
              </button>
